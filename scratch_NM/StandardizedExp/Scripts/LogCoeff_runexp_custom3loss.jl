...
##Case 5: S2R with ISO
ARGS_buffer = ["reg", "apd", "iso", "scratch_NM/StandardizedExp/Nx64/", "full_3losstest", "Full+Eps"]
ENV_buffer= "1000"
numfile = Base.parse(Int, ENV_buffer)
println(numfile, ARGS_buffer[1], ARGS_buffer[2])
...
optim_settings = Dict([("iterations", 1000), ("norm", false), ("minmethod", ConjugateGradient())])
recon_settings = Dict([("log", logbool), ("Invcov_matrix", ARGS_buffer[6]), ("optim_settings", optim_settings), ("eps_value_sfd", 1e-5), ("eps_value_init", 1e-5)]) #Add constraints
...
#Construct SFD CoeffMask
NJ, NL = length(filter_hash["j_value"]), length(filter_hash["theta_value"])
if dhc_args[:iso]
    coeff_masksfd = falses(2+S1iso+S2iso)
    #S1iso
    #=
    start = 2
    incl = S1iso
    coeff_masksfd[start+1:start+incl] .= true #S1iso
    start = start+incl
    =#
    start = 2+S1iso
    #J*J*L, J
    incl = (NJ*NJ*NL) + (NJ)
    coeff_masksfd[start+1 : start+incl] .=true
    start = start+incl
    #J exclude
    skip = NJ
    start = start + skip
    ...
    start = start + skip
    #ϕΩ, ΩΩ incl, Ωϕ excl
    coeff_masksfd[start+1] = true
    coeff_masksfd[start+3] = true
    @assert (start+3) == (S2iso+S1iso+2) println(start+3, S2iso)
...
#Construct Init CoeffMask
if dhc_args[:iso]
    coeff_maskinit = falses(2+S1iso+S2iso)
    #S1iso: J
    #=
    start = 2
    Jmask = convert(Array{Bool}, [false, false, true, true])
    Jmasksq = Jmask * Jmask'
    incl = 4
    coeff_masksfd[start+1:start+incl] .= Jmask #S1iso
    start = start+incl[]
    coeff_maskinit[start+1] = true
    start = start+2
    =#
    Jmask = convert(Array{Bool}, [false, true, true, true])
    Jmasksq = Jmask * Jmask'
    start = 2+S1iso
    #S2iso
    #JJL
    cmjjl = falses(NJ, NJ, NL)
    for l=1:NL
        cmjjl[:, :, l] .= Jmasksq
    end
    incl = NJ*NJ*NL
    coeff_maskinit[start+1:start+incl] .= cmjjl[:]
    start = start + incl
    #Jϕ
    incl=4
    coeff_maskinit[start+1:start+incl] .= Jmask
    excl = 4
    start = start+incl+excl
    coeff_maskinit[start+1] = true #ϕϕ
...
fcov = (dbnocffs .- fmean)' * (dbnocffs .- fmean) ./(size(dbnocffs)[1] -1)
cond(fcov[coeff_masksfd, coeff_masksfd])
...
p10 = heatmap(ssmoothsel[JS1ind], title="White Noise Init ϕ=" * string(smoothphi) * "Ω=" * string(smoothomg), clims=slims, c=cg)
p = plot(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, layout=(5, 2), size=(1800, 2400))
savefig(p, "scratch_NM/NewWrapper/5-14/Case5c.png")
...
##Case 4 5/14 Folder: Adding S2R: No Iso
ARGS_buffer = ["reg", "apd", "noiso", "scratch_NM/StandardizedExp/Nx64/", "full_3losstest", "Full+Eps"]
ENV_buffer= "1000"
numfile = Base.parse(Int, ENV_buffer)
println(numfile, ARGS_buffer[1], ARGS_buffer[2])
...
p10 = heatmap(ssmoothsel[JS1ind], title="Smooth Init ϕ=" * string(smoothphi) * "Ω=" * string(smoothomg), clims=slims, c=cg)
p = plot(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, layout=(5, 2), size=(1800, 2400))
savefig(p, "scratch_NM/NewWrapper/5-14/Case4g.png")

##Is Logcoeff similar to Gaussian draws or not??
#Final:

dbnfile = load("scratch_NM/SavedCovMats/reg_apd_noiso_logcoeff.jld2")
dbncoeffs = dbnfile["dbncoeffs"]
dmean = mean(dbncoeffs, dims=1)
cmask = falses(1192)
cmask[37:end] .= triu(trues(34, 34))[:]
dcov = (dbncoeffs .- dmean)' * (dbncoeffs .- dmean) ./9999

gap = MvNormal(dmean[cmask], dcov[cmask, cmask] + I*1e-5)
gapsamps = rand(gap, 1000)
p=  scatter(gapsamps[1, :], gapsamps[2, :], label="Gap")
scatter!(dbncoeffs[:, cmask][:, 1], dbncoeffs[:, cmask][:, 2], label="True")

cond(dcov  + I*1e-5)


##Case 5: S2R with ISO Cases d-e
ARGS_buffer = ["reg", "apd", "iso", "scratch_NM/StandardizedExp/Nx64/", "full_3losstest", "Full+Eps"]
ENV_buffer= "1000"
numfile = Base.parse(Int, ENV_buffer)
println(numfile, ARGS_buffer[1], ARGS_buffer[2])
...
println("Loss Term 1: SFDTarg")
println("True", loss1(true_img))
println("Init", loss1(white_noise))
println("Recon", loss1(recon_img))

println("Loss Term 2: SmoothedInitTarg")
println("True", loss2(true_img))
println("Init", loss2(white_noise))
println("Recon", loss2(recon_img))

println("Loss Term 3: Reg")
println("True", loss3(true_img))
println("Init", loss3(white_noise))
println("Recon", loss3(recon_img))
...
p4= heatmap(apodizer(white_noise), title="Init Img", clim=clim)
p5= heatmap(apodizer(imfilter(init, Kernel.gaussian(0.8))), title="Smoothed init", clim=clim)
residual_itinit_true = apodizer(white_noise)- apodizer(true_img)
maxlim = maximum([abs(minimum(residual_itinit_true)), maximum(residual_itinit_true)])
rlim = (-maxlim, maxlim)
cg = cgrad([:blue, :white, :red])
p8 = heatmap(apodizer(white_noise)- apodizer(true_img), title="Residual: Iteration Starting Point - True", c=cg, clims=rlim)
residual_itres_true = apodizer(recon_img)- apodizer(true_img)
maxlim = maximum([abs(minimum(residual_itres_true)), maximum(residual_itres_true)])
rlim = (-maxlim, maxlim)
cg = cgrad([:blue, :white, :red])
p7 = heatmap(apodizer(apdsmoothed)- apodizer(true_img), title="Residual: SmoothedInit - True",  c=cg, clims=rlim)
p6 = heatmap(apodizer(recon_img) - apodizer(true_img), title="Residual: Recon - True", c=cg, clims=rlim)

#struesel = Data_Utils.fnlog(DHC_compute_wrapper(true_img, filter_hash, norm=false; dhc_args...))
#sinitsel = Data_Utils.fnlog(abs.(DHC_compute_wrapper(white_noise, filter_hash, norm=false; dhc_args...)))
#ssmoothsel = Data_Utils.fnlog(DHC_compute_wrapper(imfilter(init, Kernel.gaussian(0.8)), filter_hash, norm=false; dhc_args...))
#sreconsel = Data_Utils.fnlog(DHC_compute_wrapper(recon_img, filter_hash, norm=false; dhc_args...))
slims = (minimum(struesel[JS1ind]), maximum(struesel[JS1ind]))
...
#p8 = heatmap(struesel[JS1ind], title="True Coeffs ϕ=" * string(truephi) * "Ω=" * string(trueomg) , clims=slims, c=cg)
#p9 = heatmap(sreconsel[JS1ind], title="Recon Coeffs ϕ=" * string(reconphi) * "Ω=" * string(reconomg), clims=slims, c=cg)
#p10 = heatmap(ssmoothsel[JS1ind], title="Smoothed Init ϕ=" * string(smoothphi) * "Ω=" * string(smoothomg), clims=slims, c=cg)
p = plot(p1, p2, p3, p4, p5, p6, p7, p8, layout=(4, 2), size=(1800, 2400))
savefig(p, "scratch_NM/NewWrapper/5-14/Case5a_new.png")
...
rlim = (-maxlim, maxlim)
cg = cgrad([:blue, :white, :red])
p6 = heatmap(apodizer(recon_img)- apodizer(true_img), title="Residual: Recon - True", clims=rlim, c=cg)
p7 = heatmap(apodizer(imfilter(init, Kernel.gaussian(0.8)))- apodizer(true_img), title="Residual: SmoothedInit - True", clims=rlim, c=cg)
...
p10 = heatmap(ssmoothsel[JS1ind], title="Smooth Init ϕ=" * string(smoothphi) * "Ω=" * string(smoothomg), clims=slims, c=cg)
p = plot(p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, layout=(5, 2), size=(1800, 2400))
savefig(p, "scratch_NM/NewWrapper/5-3/Case3f_new.png")
Visualization.plot_synth_QA(apodizer(true_img), apodizer(init), apodizer(recon_img), fname="scratch_NM/NewWrapper/4-28/1000_C5_S2sfd_highjinit.png")
