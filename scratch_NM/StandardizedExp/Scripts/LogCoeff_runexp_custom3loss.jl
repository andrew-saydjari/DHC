...
savdict = Dict([("logbool", logbool), ("apdbool", apdbool), ("isobool", isobool),
...
sampmean = mean(truesamps, dims=1)
EPS=1e-5
sampcov = ((truesamps .- sampmean)' * (truesamps .- sampmean) ./ (size(truesamps)[1] - 1)) + Diagonal(ones(1192)*EPS)eps_samp = randn(1000,
rsamps = sampmean .+ (cholesky(sampcov).U * eps_samp')'
...
covsqinv = cholesky(inv(Symmetric(sampcov))).U
whitened_ms = (covsqinv * (truesamps .- sampmean)')'
p = scatter(whitened_ms[:, 3], whitened_ms[:, 4], label="Whitened Samples from the True Coeff Distribution")
