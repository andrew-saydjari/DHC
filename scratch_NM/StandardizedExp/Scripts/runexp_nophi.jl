...
fname_save = direc * "SFDTargSFDCov/" * ARGS[1] * "_" * ARGS[2] * "_" * ARGS[3] * "/nophi_lamcorr_recon_" * string(numfile)
Nx=size(true_img)[1]
...
if dhc_args[:iso]
    error("No phi not implemented yet")
    coeff_mask = falses(2+S1iso+S2iso)
    coeff_mask[S1iso+3:end] .= true
else #Not iso
    coeff_mask = falses(2+Nf+Nf^2)
    coeff_maskS2 = trues(Nf, Nf)
    coeff_maskS2[filter_hash["phi_index"], 1:end] .= false
    coeff_maskS2[1:end, filter_hash["phi_index"]] .= false
    coeff_mask[Nf+3:end] .= coeff_maskS2[:]
end
...
recon_settings = Dict([("target_type", "sfd_dbn"), ("covar_type", "sfd_dbn"), ("log", logbool), ("GaussianLoss", true), ("Invcov_matrix", "Diagonal+Eps"),
