...
####### Testing the ISO Matrix now #########

filter_hash = fink_filter_hash_AKS(1, 8, nx=256, pc=2, Omega=true)
filt_3d = fink_filter_bank_3dizer_AKS(filter_hash, 1, nz=256, Omega3d=true)

test1 = S1_iso_matrix_AKS(filter_hash)
sum(test1,dims=1) == ones(1,50)
unique(sum(test1,dims=2))

test1 = S2_iso_matrix_AKS(filter_hash)
sum(test1,dims=1) == ones(1,2500)
unique(sum(test1,dims=2))

filter_hash = fink_filter_hash_AKS(1, 8, nx=256, pc=2, Omega=false)

test1 = S1_iso_matrix_AKS(filter_hash)
sum(test1,dims=1) == ones(1,49)
unique(sum(test1,dims=2))

test1 = S2_iso_matrix_AKS(filter_hash)
sum(test1,dims=1) == ones(1,2401)
unique(sum(test1,dims=2))

filter_hash = fink_filter_hash_AKS(1, 8, nx=256, pc=2, Omega=false)
filt_3d = fink_filter_bank_3dizer_AKS(filter_hash, 1, nz=256, Omega3d=false)

test = S1_iso_matrix3d_AKS(filt_3d)
sum(test,dims=1) == ones(1,295)
unique(sum(test,dims=2))
index1 = collect(1:43)
mask = sum(test,dims=2).==9
index1[mask[:,1]]
...
function S1_iso_matrix_AKS(fhash)
    # fhash is the filter hash output by fink_filter_hash
    # The output matrix converts an S1 coeff vector to S1iso by
    #   summing over l
    # Matrix is stored in sparse CSC format using SparseArrays.
...
function S2_iso_matrix_AKS(fhash)
    # fhash is the filter hash output by fink_filter_hash
    # The output matrix converts an S2 coeff vector to S2iso by
    #   summing over l1,l2 and fixed Δl.
    # Matrix is stored in sparse CSC format using SparseArrays.
...
filt_3d_test = filt_3d["psi_index"]

filt_3d_test[6+1,1,6+1]
