...
dhc_main = dirname(@__DIR__)*"\\main"
# put the DHC/main directory on the path so Julia can find the module
push!(LOAD_PATH, dhc_main)
using DHC_2DUtils



## Quijote data
quijote2d_dir = "C:\\Users\\JR\\Documents\\HarvardWork\\Research\\WST\\quijote\\density_field_2d\\latin_hypercube"

@time fhash = fink_filter_hash(1, 8, nx=256, pc=1, wd=1)
nfile = 10
ncoeffs = 7254

all_files  = []
all_coeffs = zeros(ncoeffs, nfile)
for i=1:nfile-1
    file = glob("*.npy", quijote2d_dir*"\\"*string(i))[1]
    push!(all_files, file)
    map  = convert(Array{Float64, 2}, npzread(file))
    coeffs = DHC_compute(map, fhash, fhash, doS2=true, doS12=true, doS20=true)
    all_coeffs[:,i] = coeffs
end
